...
function updateBatch(state: AppState, batchId: string, update: (b: Batch) => Batch): AppState {
  return {
    ...state,
    batches: state.batches.map((b) => (b.id === batchId ? update(b) : b)),
  };
}

export function addCulture(
  state: AppState,
  input: { species: string; type: CultureType; storage: StorageLocation; qty: number }
  ...
  next = logActivity(
    next,
    `${id} added`,
    `${culture.qty} ${input.type === "agar" ? "plate(s)" : "jar(s)"} · ${
      input.storage === "fridge" ? "refrigerated" : "room temp"
    }`
  );
...
export function toggleCultureStorage(state: AppState, cultureId: string): AppState {
  ...
  const next: AppState = {
    ...state,
    cultures: state.cultures.map((x) => (x.id === cultureId ? { ...x, storage } : x)),
  };
  return logActivity(
    next,
    `${c.id} moved to ${storage === "fridge" ? "refrigerator" : "room temperature"}`,
    `${c.qty} ${c.type === "agar" ? "plates" : "jar(s)"}`
  );
}
...
export function inoculateToGrain(
  state: AppState,
  cultureId: string,
  qty: number,
  unit: string
): { state: AppState; id: string } | null {
  const c = state.cultures.find((x) => x.id === cultureId);
  if (!c) return null;
  ...
  const batch: Batch = {
    ...
    location: null,
    flushes: [],
    history: [{ event: "Grain inoculated", sub: `From ${c.id} · ${qty || 1} ${unit}`, date: todayStr() }],
  };
...
export function advanceToBreak(state: AppState, batchId: string): AppState {
  ...
  if (!b) return state;
  const next = updateBatch(state, batchId, (batch) => ({
    ...batch,
    phase: "break",
    history: [...batch.history, { event: "Break & shake", sub: `${batch.qty} ${batch.qtyUnit} mixed`, date: todayStr() }],
  }));
...
export function spawnToBulk(state: AppState, batchId: string, qty: number, unit: string): AppState {
  ...
  const next = updateBatch(state, batchId, (batch) => ({
    ...
    qty: qty || batch.qty,
    qtyUnit: unit,
    history: [...batch.history, { event: "Spawned to bulk", sub: `${qty} ${unit}`, date: todayStr() }],
  }));
...
export function moveToFruiting(state: AppState, batchId: string, slotId: string): AppState {
  const b = state.batches.find((x) => x.id === batchId);
  if (!b) return state;
  if (occupantOf(state, slotId)) return state;
  const next = updateBatch(state, batchId, (batch) => ({
    ...batch,
    phase: "fruiting",
    location: slotId,
    history: [...batch.history, { event: "Moved to fruiting", sub: slotLabel(slotId), date: todayStr() }],
  }));
...
export function moveLocation(state: AppState, batchId: string, slotId: string): AppState {
  ...
  const next = updateBatch(state, batchId, (batch) => ({
    ...batch,
    location: slotId,
    history: [...batch.history, { event: "Location updated", sub: slotLabel(slotId), date: todayStr() }],
  }));
...
export function harvestFlush(state: AppState, batchId: string, freshWeight: number): AppState {
  ...
  const next = updateBatch(state, batchId, (batch) => ({
    ...batch,
    phase: "drying",
    flushes: [...batch.flushes, { n, freshWeight, dryWeight: null, harvestDate: todayStr(), driedDate: null }],
    history: [
      ...batch.history,
      { event: `Flush ${n} harvested`, sub: `${freshWeight} g fresh · drying`, date: todayStr() },
    ],
  }));
...
export function logDryWeight(state: AppState, batchId: string, dryWeight: number): AppState {
  ...
  if (!flush) return state;
  const next = updateBatch(state, batchId, (batch) => ({
    ...batch,
    phase: "fruiting",
    flushes: batch.flushes.map((f, i) =>
      i === batch.flushes.length - 1 ? { ...f, dryWeight, driedDate: todayStr() } : f
    ),
    history: [
      ...batch.history,
      { event: `Flush ${flush.n} dried`, sub: `${dryWeight} g added to inventory`, date: todayStr() },
    ],
  }));
  const withInventory: AppState = {
    ...next,
    inventory: { ...next.inventory, [b.species]: (next.inventory[b.species] || 0) + dryWeight },
    lifetimeInventory: {
      ...next.lifetimeInventory,
      [b.species]: (next.lifetimeInventory[b.species] || 0) + dryWeight,
    },
  };
...
export function retireBatch(state: AppState, batchId: string): AppState {
  ...
  const next = updateBatch(state, batchId, (batch) => ({
    ...
    phase: "done",
    location: null,
    history: [...batch.history, { event: "Batch retired", sub: "Removed from active rotation", date: todayStr() }],
  }));
